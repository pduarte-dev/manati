...
export class FileLockManager {
    constructor() {
        this.currentLocks = new Map(); // Armazena locks ativos
        this.currentFile = null;
        this.lockCheckInterval = null;
        this.activityInterval = null;
        this.ACTIVITY_UPDATE_INTERVAL = 15000; // 15 segundos (mais frequente)
        this.LOCK_CHECK_INTERVAL = 30000; // 30 segundos (mais frequente)
    }
        ...
        try {
            ...
            if (data.success) {
                ...
                data.locks.forEach(lock => {
                    this.currentLocks.set(lock.filePath, {
                        userId: lock.userId,
        ...
        fileItems.forEach(item => {
            ...
            if (lock) {
                ...
                if (isMyLock) {
                    ...
                    item.removeAttribute('title'); // Remover qualquer title
                } else {
                ...
                // Garantir que não há texto visível extra adicionado
                item.removeAttribute('data-lock-text');
    ...
    extractFileNameFromItem(item) {
        // Primeiro tentar pelo data attribute
        const filePath = item.dataset?.filePath;
        if (filePath) {
            return filePath;
        }
        
        // Alternativa: verificar se o item tem o event listener de click
    ...
    /**
     * Força o modo de visualização (preview)
     */
    setPreviewMode() {
